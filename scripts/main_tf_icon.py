...
def main():
    ...
    parser.add_argument(
        "--prompt", type=str, nargs="?", default="a professional photograph of a doggy, ultra realistic", help="the prompt to render"
    )
    ...
    if not os.path.exists(opt.ckpt):
        ...
        print(f"Checkpoint {opt.ckpt} not found, downloading from huggingface")
        os.system(f"wget -O {opt.ckpt} https://huggingface.co/stabilityai/stable-diffusion-2-1-base/resolve/main/v2-1_512-ema-pruned.ckpt")
            ...
            if file == files[-1]:
                ...
                save_image[
                    ...
                    save_image[
                        :,
                        :,
                        center_row_rm - step_height1 : center_row_rm + step_height2,
                        center_col_rm - step_width1 : center_col_rm + step_width2,
                    ].clone()
                ...
                image = Image.fromarray(
                    ((save_image / torch.max(save_image.max(), abs(save_image.min())) + 1) * 127.5)[0]
                    .permute(1, 2, 0)
